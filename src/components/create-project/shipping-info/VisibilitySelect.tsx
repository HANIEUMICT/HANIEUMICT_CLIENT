...
export default function VisibilitySelect() {
  ...
  useEffect(() => {
    if (projectData.projectStatus && projectData.projectStatus === 'PRIVATE') {
      setIsPublic(false)
    } else {
      setIsPublic(true)
    }
    ...
    <div className="flex flex-col gap-y-[16px]">
      <section className="flex flex-col gap-y-2">
        <p className="sub2 flex gap-x-1">
          견적서 공개 여부<span className="text-conic-red-30">*</span>
        </p>
      ...
      {isPublic && (
        <section className="flex flex-col gap-y-2">
          <p className="sub2 flex gap-x-1">
            견적서를 공개할 공장 <span className="text-conic-red-30">*</span>
          </p>
