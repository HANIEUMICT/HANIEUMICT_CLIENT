...
import { CompanyDetailInfoType, CompanyType, RecommendCompanyType } from '@/type/company'
 ...
 * 추천 공급 업체 목록 조회
 */
export const getCompanyProfileRecommend = async (): Promise<ApiResponse<RecommendCompanyType[]>> => {
  const response = await serverAuthorizedFetch(`${process.env.NEXT_PUBLIC_BASE_URL}/v1/company/profiles/recommend`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    },
